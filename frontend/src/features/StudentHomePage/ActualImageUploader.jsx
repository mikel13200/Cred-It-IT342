...
export default function ActualImageUploader({onContinue}) {
  const [images, setImages] = useState([]); // [{id, src, name}]
  const dragIndex = useRef(null);
  const fileInputRef = useRef(null);
  

  const toId = () => `${Date.now()}-${Math.random().toString(36).slice(2)}`;
          ...
          new Promise((resolve) => {
            ...
            reader.onload = (e) =>
              resolve({
                ...
                src: e.target.result,
                name: file.name,
                file, // ✅ include original File object
              });
  ...
  // Optional: also allow dropping files onto the box
  const onDropFiles = (e) => {
    e.preventDefault();
    ...
    if (onContinue && typeof onContinue === "function") {
      onContinue(images); // Send images back to parent
    }
  };
  ...
  return (
    <div className="bg-gradient-to-r from-blue-600 to-indigo-600 p-8 rounded-2xl text-white text-center">
      ...
      {/* Process button (reveals only when 1+) */}
      {images.length >= 1 && (
        <div className="mt-6">
          <button
            type="button"
            className="bg-white text-blue-600 font-semibold px-6 py-3 rounded-lg shadow-lg hover:bg-blue-100 inline-flex items-center"
